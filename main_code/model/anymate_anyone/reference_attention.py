...
class ReferenceNetAttention():
    ...
    def register_reference_hooks(
        ...
        if do_classifier_free_guidance:
            uc_mask = ( 
                torch.Tensor([1] * batch_size * num_images_per_prompt * 16 + [0] * batch_size * num_images_per_prompt * 16) # 在batch层面上mask！！！
                .to(device) 
                .bool()
            )
        else:
        ...
        def hacked_basic_transformer_inner_forward(
            self,
            hidden_states: torch.FloatTensor,   # 输入的原始hidden_states!
            attention_mask: Optional[torch.FloatTensor] = None,
            encoder_hidden_states: Optional[torch.FloatTensor] = None,  # 来自于reference信息？？？
            ...
            if self.only_cross_attention:
                ...
                if MODE == "write":
                    # 注意这里的reference信息来自于attn之前的norm结果
                    self.bank.append(norm_hidden_states.clone())    # self.bank只保存forward的输入？？？因为可以用同一个attn1进行处理
                    attn_output = self.attn1(
                        norm_hidden_states,
                        encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,
                        attention_mask=attention_mask,
                ...
                if MODE == "read":
                    ...
                    ref_hidden_states = torch.cat(self.bank, dim = 1)   # B, L * M, C

                    attn_output_ref_attn = self.attn_ref(
                        norm_hidden_states,
                        encoder_hidden_states=ref_hidden_states,
                        attention_mask=attention_mask,
                        **cross_attention_kwargs,
                    )
                    hidden_states = attn_output_ref_attn + hidden_states
                    
                    
                    
                    attn_output_attn_1 = self.attn1(norm_hidden_states, 
                    ...
                    if self.attn2 is not None:
                        ...
                        hidden_states = (
                            self.attn2(
                                norm_hidden_states, encoder_hidden_states=encoder_hidden_states, attention_mask=attention_mask  # encoder_hidden_states来自于原始的text cross
                            )
                            + hidden_states # 
                        )
        ...
        # 注入的时机（midup, full）
        # 之后可以实现类似于controlnet的注入顺序（referencenet encoder -> unet decoder）
