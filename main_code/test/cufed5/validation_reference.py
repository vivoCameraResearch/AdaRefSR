...
from main_code.test.cufed5.cufed5dataset import CUFED5ValidDataset
...
def main(args, device):
    ...
    dataset_val = CUFED5ValidDataset(opt = dataset_config.dataset)
    ...
    for step, batch_val in tqdm(enumerate(dl_val)):
        ...
        with torch.no_grad():
            ...
            x_src_ram = ram_transforms(x_src).to(device)
            x_ref_ram = ram_transforms(ref_img_val).to(device)
        ...
        ref_height, ref_width = batch_val["ref_height"], batch_val["ref_width"]
        ...
        x_tgt_pred = x_tgt_pred[:,:height,:width].clamp_(0.0, 1.0)
        output_pil = transforms.ToPILImage()(x_tgt_pred)
        output_pil = output_pil.resize((gt_width, gt_height), Image.BICUBIC)
        
        image_name_out = image_name.replace("LR", "HR")
        outf = os.path.join(valid_image_dir_pred, image_name_out)
        output_pil.save(outf)
        
        ref_predict = ref_predict[:,:ref_height,:ref_width].clamp_(0.0, 1.0)
        output_pil_ref = transforms.ToPILImage()(ref_predict[0])
        image_name_out = image_name.replace("LR", "Ref")
        outf_ref = os.path.join(ref_image_dir_pred, image_name_out)
        output_pil_ref.save(outf_ref)
        
        # todo 1: get output image
        if args.save_lr_pred_ref:
            x_tgt = x_tgt[0]    # [c, h1, w1]
            x_src_unpadded = x_src[0][:,:height,:width]    # [c, h2, w2]
            
            # ref_img_val_unpadded = ref_img_val[0][:, :height, :width]    # [c, h2, w2]
            ...
            ref_img_val_resized = F.interpolate(ref_img_val, size=(h1, w1), mode='bilinear', align_corners=False).squeeze(0)
