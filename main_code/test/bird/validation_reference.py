...
from main_code.test.bird.RefDataset import RefTestDataset
...
if __name__ == "__main__":
    ...
    yaml_path = "your_code_path/configs/base_config.yaml" 
    if os.path.exists(yaml_path):
        # 加载 YAML 配置
        yaml_cfg = OmegaConf.load(yaml_path)
        # 将 argparse 的 Namespace 对象转换为 OmegaConf 对象
        args_cfg = OmegaConf.create(vars(args))
        updated_cfg = OmegaConf.merge(args_cfg, yaml_cfg)
        args = updated_cfg
        print(f">>> Loaded config from {yaml_path}")
    else:
        print(f">>> Warning: {yaml_path} not found, using command line args only.")
