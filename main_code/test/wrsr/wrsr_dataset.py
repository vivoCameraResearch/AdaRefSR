...
class WRSRValidDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(WRSRValidDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if 'wrsr' in lr_path:
            ref_im_path = os.path.join(self.ref_dir, os.path.basename(image_name).replace('LR','ref'))
            img_ref = Image.open(ref_im_path).convert("RGB")
            gt_path = os.path.join(self.gt_dir, img_id + "_HR.png")
