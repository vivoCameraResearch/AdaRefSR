...
class RefTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get dinov2 model input
        img_lr_dino = dino_transforms(to_pil_image(img_lr)) # dino接受的输入是pil image (0, 255)
        img_ref_dino = dino_transforms(to_pil_image(img_ref))
        
        # to tensor and input 
        img_lr = torch.clamp(img_lr, 0, 1.0)
        img_ref = torch.clamp(img_ref, 0, 1.0)
        
        return_d = {'lr': img_lr, 'ref': img_ref, 'gt': img_gt, 'lr_path': lr_path, 'lr_name': image_name, "height": target_height, "width": target_width, 'lr_ram': img_lr_ram, 'ref_ram': img_ref_ram, 'lr_dino': img_lr_dino, 'ref_dino': img_ref_dino}
