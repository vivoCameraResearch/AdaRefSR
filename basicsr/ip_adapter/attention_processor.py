...
class IPAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        # 打印一下范数平方，看看hidden_states的作用效果
        print(f"ip key: {torch.norm(ip_key)}, ip value: {torch.norm(ip_value)}") 
              
        print(f"ip_hidden_states: {torch.norm(ip_hidden_states)}, hidden_states: {torch.norm(hidden_states)}, ratio: {torch.norm(ip_hidden_states) / torch.norm(hidden_states)}")
...
def init_attn_proc(unet, ip_adapter_tokens=16, use_lcm=False, use_adaln=True, use_external_kv=False):
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None: # todo self attention???
            if use_external_kv:
            ...
            if use_adaln:
                layer_name = name.split(".processor")[0]    # todo layer name, can use k, v weight here???
                if use_lcm:
                    weights = {
                        "to_k_ip.weight": unet_sd[layer_name + ".to_k.base_layer.weight"],  # add model weight!!! # todo
                        "to_v_ip.weight": unet_sd[layer_name + ".to_v.base_layer.weight"],
                    }
                else:
                    weights = {
                        "to_k_ip.weight": unet_sd[layer_name + ".to_k.weight"], # 使用初始化kv权重
                        "to_v_ip.weight": unet_sd[layer_name + ".to_v.weight"],
                    }
                    
                # todo ----------------------- ip image cross attention added!!! ---------------------- # todo 取消time-aware!
