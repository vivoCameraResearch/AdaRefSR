...
def parse_args_paired_training(input_args=None):
   ...
   """
    ...
    parser.add_argument("--lora_rank_unet", default=32, type=int)   # default is 32, 16
    ...
    parser.add_argument("--log_name", default="reference-sr", required=True)
    parser.add_argument("--log_file", default="logs/reference-sr.log", required=True)
    parser.add_argument("--summer_writer", default="summer_writer", required=True)
    # parser.add_argument("--accumulation_steps",default=4, help="Accumulation gradient for training!")
    parser.add_argument(
    ...
    parser.add_argument("--add_channel_factor", action="store_true", help="Whether to add channel_factor")
    parser.add_argument("--add_deg_mlp", action="store_true", help="Whether to add deg mlp")
    
    parser.add_argument("--add_spatial_align_net", action="store_true", help="Whether to add spatial_align_net")
    ...
    parser.add_argument("--visualize_nums", type=int, default=8)
    parser.add_argument("--train_attention_lora", action="store_true", help="Whether to train s3diff' attention lora")
    ...
    parser.add_argument("--lambda_contrast", default=0.01, type=float)
...
# todo 更改代码，读取图像：gt+reference，解析路径，并自动分配合理的reference数据集
# 先写好训练数据集的处理逻辑
# @DATASET_REGISTRY.register(suffix='basicsr')

def split_string(input_string):
    # 查找 "raw" 的位置
    raw_index = input_string.rfind("/")
    
    if raw_index == -1:
        raise ValueError("The string does not contain 'raw_'")

    # 拆分为两部分
    part1 = input_string[:raw_index + len("raw_")]
    part2 = input_string[raw_index + len("raw_"):]
...
# just Gaussian, not crop
"""
降质处理管道，修改自 Real-ESRGAN

Args:
 configs (object): 配置对象，包含降质参数
 batch (dict): 输入批次，包含真实图像和卷积核
 device (torch.device): 计算设备
 val (bool, optional): 是否验证模式，默认为 False
 use_usm (bool, optional): 是否使用 USM 锐化，默认为 False
 resize_lq (bool, optional): 是否调整低质量图像大小，默认为 True
 random_size (bool, optional): 是否随机调整大小，默认为 False

Returns:
 tuple: 包含低质量图像、真实图像和原始低质量图像的元组，均转移至指定设备
"""
def degradation_proc_valid(configs, batch, device, val=False, use_usm=False, resize_lq=True, random_size=False):
...
if __name__ == "__main__":
    ...
    config = json.load("configs/sr.yaml")
    dataset_train = PairedDataset(config.train)
    batch_size = 1
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=1, shuffle=True, num_workers=4)
